function [dSbus_dV1, dSbus_dV2] = dSbus_dV_mod(Ybus, V, va_var, vm_var, vcart)
...
%     diagIbus    = sparse(1:n, 1:n, Ibus, n, n);
    diagV_va       = sparse(va_var, va_var, V(va_var), n, n);
    diagV_vm       = sparse(vm_var, vm_var, V(vm_var), n, n);
    diagIbus_va    = sparse(va_var, va_var, Ibus(va_var), n, n);
    diagIbus_vm    = sparse(vm_var, vm_var, Ibus(vm_var), n, n);
    if ~vcart
%         diagVnorm   = sparse(1:n, 1:n, V./abs(V), n, n);
        diagVnorm   = sparse(vm_var, vm_var, V(vm_var), n, n);
    end
else                        %% dense version
    diagV       = diag(V);
    diagIbus    = diag(Ibus);
...
if vcart
    ...
    dSbus_dV1 = 1j * diagV_va * conj(diagIbus_va - Ybus * diagV_va);                  %% dSbus/dVa
    dSbus_dV2 = diagV_vm * conj(Ybus * diagVnorm) + conj(diagIbus_vm) * diagVnorm;    %% dSbus/dVm
end